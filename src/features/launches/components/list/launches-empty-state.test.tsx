...
describe("LaunchesEmptyState", () => {
  it("renders generic message when no active filters", () => {
    ...
    expect(screen.getByText("There are no launches available at the moment.")).toBeInTheDocument();
    expect(screen.queryByRole("button")).not.toBeInTheDocument();
  });

  it("renders filter message and reset button when filters are active", () => {
    const onReset = vi.fn();
    renderWithProviders(<LaunchesEmptyState hasActiveFilters={true} onReset={onReset} />);
